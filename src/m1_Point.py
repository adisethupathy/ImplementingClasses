...
class Point(object):
    ...
    def __repr__(self):
        printed = ('Point({}, {})'.format(self.x, self.y))
        return printed

    def clone(self):
        twin = Point(self.x, self.y)
    ...
    def move_to(self, nx, ny):
        ...
        self.y = ny
        self.moves += 1
        distx = self.x - self.old_x
        disty = self.y - self.old_y
        self.change += distx + disty
    ...
    def get_distance_from(self, point):
        changex = self.x - point.x
        changey = self.y - point.y

        totaldistance = math.sqrt((changex ** 2) + (changey ** 2))

        return totaldistance


    def get_distance_from_start(self):
        compx = math.fabs(self.x - self.old_x)
        compy = math.fabs(self.y - self.old_y)

        totaldistance = math.sqrt((compx **2) + (compy ** 2))
    ...
    def halfway_to(self, p2):
        ...
        dx = math.fabs((self.x - p2.x)/2)
        dy = math.fabs((self.y - p2.y)/2)

        half = Point(self.x + dx, self.y + dy)
